...
if strcmp(get(handles.pressure_measurement, 'Checked'), 'on')
    ...
    if strcmp(get(handles.include_zero, 'Checked'), 'on')
        set(handles.stats01, 'String', ['Mean: ' num2str(mean(mean(mag_roi(~isnan(mag_roi) & mag_roi>=str2double(get(handles.noise_floor_edit,'String'))))))...
            ' MPa  |  Max: ' num2str(max(max(mag_roi(~isnan(mag_roi) & mag_roi>=str2double(get(handles.noise_floor_edit,'String')))))) ' MPa']);
        set(handles.stats02, 'String', ['Min: ' num2str(min(min(mag_roi(~isnan(mag_roi) & mag_roi>=str2double(get(handles.noise_floor_edit,'String'))))))...
            ' MPa  |  Sum: ' num2str(sum(sum(mag_roi(~isnan(mag_roi) & mag_roi>=str2double(get(handles.noise_floor_edit,'String')))))*sensel_area) ' N']);
    else
        set(handles.stats01, 'String', ['Mean: ' num2str(mean(mean(mag_roi(~isnan(mag_roi) & mag_roi ~= 0 & mag_roi>=str2double(get(handles.noise_floor_edit,'String'))))))...
            ' MPa  |  Max: ' num2str(max(max(mag_roi(~isnan(mag_roi) & mag_roi>=str2double(get(handles.noise_floor_edit,'String')))))) ' MPa']);
        set(handles.stats02, 'String', ['Min: ' num2str(min(min(mag_roi(~isnan(mag_roi) & mag_roi ~= 0 & mag_roi>=str2double(get(handles.noise_floor_edit,'String'))))))...
            ' MPa  |  Sum: ' num2str(sum(sum(mag_roi(~isnan(mag_roi) & mag_roi>=str2double(get(handles.noise_floor_edit,'String')))))*sensel_area) ' N']);
    end
    set(handles.stats03, 'String', ['Total Contact Area: ' num2str(sensel_area*length(mag_roi(mag_roi>0 & mag_roi>=str2double(get(handles.noise_floor_edit,'String'))))) ' mm^2']);
